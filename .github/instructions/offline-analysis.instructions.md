...
- When using review mode, always include `review.status` as advisory simulated-model evidence, evidence class/scope/sufficiency, top watch/regression findings, caveat, and artifact path.
...
- **Review Model**: Use `review.status` (`ok` | `watch` | `regression`) and `review.findings` as the review contract. Treat `ok` as "no modeled regression detected in reviewed scenarios," not approval.
...
- **Structured Contract**: Review artifact payloads are not schema-versioned. Machine consumers should key off `review.status`, `review.evidence`, and `review.findings` as advisory simulated-model evidence, and ignore unrelated report fields.
- **Structured Output**: Compare and matrix review text should use the same section order (`METRICS`, `SIMULATED PROGRESSION REVIEW`, `METADATA`) so evidence mode does not change report reading flow.
- Treat `review.status=ok` as advisory simulated-model evidence, not approval. Foundational tuning changes still require matrix evidence and targeted e2e validation.
