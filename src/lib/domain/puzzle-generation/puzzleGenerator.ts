...
} from './skillBasedPuzzleSettings.ts'
import {
...
function resolvePuzzlePartsRequest(
	...
	const operatorSkill = quiz.skillByOperator[operator]
...
function resolveResolvedOperatorPuzzleSettings(
	quiz: Quiz,
	...
	const adaptiveSkill = quiz.skillByOperator[activeOperator]

	const adaptiveSettings = resolveOperatorPuzzleSettings(
		activeOperator,
		adaptiveSkill,
		normalizedDifficulty,
		baseSettings.range,
...
export function computeDifficultyWindow(
	...
	const { operator, skill, skillByOperator, applyWeakOperatorBoost } = context
	...
	if (applyWeakOperatorBoost && skillByOperator != null) {
		const avgSkill =
			skillByOperator.reduce((sum, s) => sum + s, 0) / skillByOperator.length
