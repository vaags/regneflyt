...
import { progressionInternals } from './skillModel.ts'
import { getActiveTuning } from './adaptiveTuning.ts'
 ...
 */
export function sanitizeOperatorSkillMap(value: unknown): OperatorSkillMap {
	if (!isOperatorTuple(value))
		return cloneOperatorTuple(defaultOperatorSkillMap)
...
function isOperatorTuple(value: unknown): value is OperatorTuple<unknown> {
	return (
		Array.isArray(value) && value.length === progressionInternals.operatorCount
	)
}
...
function clampDuration(
	...
	return Math.max(
		progressionInternals.minDurationSeconds,
