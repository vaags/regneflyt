...
type ActiveInfo = { tag: string; id: string | null; class: string | null; visible: boolean }

for (const colorScheme of ['light', 'dark'] as const) {
  test.describe(`a11y extended (${colorScheme})`, () => {
    test(`menu and quiz screens have no critical or serious accessibility violations`, async ({ page }) => {
      ...
      let criticalOrSeriousViolations = accessibilityScanResults.violations.filter((violation) =>
      ...
      if (startButtons > 0) {
        ...
        criticalOrSeriousViolations = accessibilityScanResults.violations.filter((violation) =>
          ['critical', 'serious'].includes(violation.impact ?? '')
        )
    ...
    test('basic keyboard focus flow (first interactive elements)', async ({ page }) => {
      await page.emulateMedia({ colorScheme })
      await page.goto('/')
      await page.waitForLoadState('networkidle')
      ...
      for (let i = 0; i < tabSteps; i++) {
        ...
        const active = await page.evaluate<ActiveInfo | null>(() => {
          ...
          return { tag: el.tagName, id: el.id || null, class: el.className || null, visible }
        })
    ...
    test('share panel: axe scan and focus order', async ({ page }) => {
      ...
      const shareToggle = actionRow.getByRole('button', { name: /^Del$/i }).first()
      ...
      const criticalOrSeriousViolations = accessibilityScanResults.violations.filter((violation) =>
        ['critical', 'serious'].includes(violation.impact ?? '')
      )
      expect(criticalOrSeriousViolations).toEqual([])
      ...
      await page.keyboard.press('Tab')
      const shareButton = sharePanel.getByRole('button', { name: /^Del$/i }).first()
