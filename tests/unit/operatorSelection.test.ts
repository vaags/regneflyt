...
import { resolveOperator } from '#lib/domain/puzzle-generation/operatorSelection.ts'
...
describe('operatorSelection', () => {
	it('returns selected operator directly when not All', () => {
		const skillMap: OperatorSkillMap = [10, 20, 30, 40]
	...
	it('samples all operators in non-adaptive All mode', () => {
		const skillMap: OperatorSkillMap = [50, 50, 50, 50]
	...
	it('uses weighted adaptive All mode to favor weaker operators', () => {
		...
		const skillByOperator: OperatorSkillMap = [100, 0, 0, 0]
	...
	it('is near-uniform in adaptive All mode when skills are equal', () => {
		...
		const skillByOperator: OperatorSkillMap = [25, 25, 25, 25]
